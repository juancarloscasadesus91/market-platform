    ...
    <x-card class="relative">
        ...
        <div class="mb-3 grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
            ...
            <div>
                ...
                <p class="mt-0.5 text-xs text-slate-600">SYMBOL + YYMMDD + C/P + STRIKE &nbsp;·&nbsp; max 20</p>
            </div>
            ...
            <div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input type="number" x-model.number="dteStrikes" min="1" max="15"
                           class="w-12 px-1.5 py-1 text-xs bg-slate-700/50 text-white border border-slate-600 rounded focus:outline-none focus:border-purple-500 text-center" />
                    <button @click="runDteScanner()"
                            :disabled="isDteScanning || contracts.length >= 20"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                        ...
                        <div class="bg-slate-900/50 rounded p-3 border-l-4 border-emerald-500">
                            ...
                            <p class="text-xs text-slate-500" x-show="marketSentiment.upsideTarget">
                                Based on $<span x-text="(callStats.totalPremium / 1000).toFixed(0)"></span>K CALL premium
                            </p>
                        </div>
                        ...
                        <div class="bg-slate-900/50 rounded p-3 border-l-4 border-rose-500">
                            ...
                            <p class="text-xs text-slate-500" x-show="marketSentiment.downsideTarget">
                                Based on $<span x-text="(putStats.totalPremium / 1000).toFixed(0)"></span>K PUT premium
                            </p>
                        </div>
                    </div>
                    ...
                    <div x-show="showCallDetails" x-transition class="mt-4 space-y-4">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            ...
                            <div class="p-2 bg-blue-500/10 border border-blue-500/20 rounded">
                                ...
                                <p class="text-sm font-semibold text-blue-400" x-text="activeCallContract?.detailedStats.midVolume || 0"></p>
                ...
                <div x-show="activePutContract !== null">
                    ...
                    <div x-show="showPutDetails" x-transition class="mt-4 space-y-4">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            ...
                            <div class="p-2 bg-blue-500/10 border border-blue-500/20 rounded">
                                <p class="text-xs text-slate-400">Mid</p>
                                <p class="text-sm font-semibold text-blue-400" x-text="activePutContract?.detailedStats.midVolume || 0"></p>
...
function optionMonitor() {
    return {
        ...
        calculateTargets() {
            ...
            // Calculate premium-weighted average strike for calls
            ...
            this.callContracts.forEach(contract => {
                ...
                const weight = contract.lightStats.totalPremium;
                if (strike && weight > 0) {
            ...
            // Calculate premium-weighted average strike for puts
            ...
            // Calculate probabilities based on premium ratio
            const callNet = this.callStats.netPremium;
            const putNet = this.putStats.netPremium;
        ...
        handleBulkLoaded(detail) {
            ...
            for (const symbol of symbols) {
                if (this.contracts.length >= 20) break;
        ...
        handleLevelOne(data) {
            ...
            data.content.forEach(quote => {
                ...
                // Always use processFullQuote for the first tick and for selected contracts
                if (!contract.seenFirstTick || contractIndex === this.selectedCallIndex || contractIndex === this.selectedPutIndex) {
        ...
        determineSide(quote, price) {
            ...
            // Dynamic threshold based on spread size
            let thresholdRatio;
            if (spread < 0.10) {
                thresholdRatio = 0.15; // Tight spread - very sensitive
            } else if (spread <= 0.50) {
                thresholdRatio = 0.20; // Normal spread - balanced
            } else {
                thresholdRatio = 0.25; // Wide spread - more conservative
            }

            const threshold = spread * thresholdRatio;
